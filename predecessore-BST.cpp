...
class BST
{
   public:
      ...
      H predecessore(H value)
      {
         Node<H> *check = search(value);
         if(check->getLeft())
         {
            check = massimo(check->getLeft());
            return check->getData();
         }
         else
         {
            Node<H> *parent = check->getFather();
            while(parent && check == parent->getLeft())
            {
               check = parent;
   ...
   private:
      ...
      Node<H> * massimo(Node<H> *check)
      {
         while(check->getRight())
            check = check->getRight();
         return check;
      }
};
