...
class BST
{
   public:
      ...
      int bilancia(H value)
      {
         Node<H> *check = search(value);
         return abs(_bilancia(check->getLeft()) - _bilancia(check->getRight()));
      }
   ...
   private:
      ...
      int _bilancia(Node<H> *check)
      {
         if(!check)
            return 0;
         else
            return 1 + _bilancia(check->getLeft()) + _bilancia(check->getRight());
      }
};
